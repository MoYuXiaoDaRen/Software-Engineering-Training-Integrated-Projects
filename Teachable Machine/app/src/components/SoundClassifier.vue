<template>
  <div v-title data-title="Sound">
    <div id = "class1" overflow:hidden>
      <h3>第一类音频</h3>
      <form id="class1-form" @change="setImg($event)">
        <input type="file" name="sound" accept="audio/mp3" id='1'/>
        <br>
        <br>
        <img src="" id='img_1' class="pic">
        <br>
        <br>
        <input type="file" name="sound" accept="audio/mp3" id='2'/>
        <br>
        <br>
    ...
    <div id = "class2">
      <h3>第二类音频</h3>
        ...
        <input type="file" name="sound" accept="audio/mp3" id='3'/>
        <br>
        <br>
        <img src="" id='img_3' class="pic">
        <br>
        <br>
        <input type="file" name="sound" accept="audio/mp3" id='4'/>
        <br>
        <br>
...
export default {
  ...
  methods: {
    ...
    setImg (e) {
      let target = e.target
      $('#img_' + target.id).attr('src', '/static/SoundWave 声波.png')
    },
    // 提交信息到后台
    submit () {
      const path = 'http://127.0.0.1:5000/soundsClassifier'
      // const path = 'http://192.168.1.112:5000/soundsClassifier'
      ...
      if (isImgEnough === false && isLabelEnough === false) {
        ...
        for (let i = 1; i <= 4; i++) {
          ...
          uploaddata.append('sound_' + i.toString(), file)
        }
        for (let i = 1; i <= 2; i++) {
